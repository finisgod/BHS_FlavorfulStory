...
public static class TileGridFactory
{
    ...
    {
        Tile newTile = Activator.CreateInstance(tileType) as Tile; // «аменить на более производительный вариант
    ...
    public static Tile[,] CreateTileArray(Type tileType, int Width, int Length)
    {
        ...
        for (int i = 0; i < Width; i++)
        {
            ...
            {
                newTileArray[i, j] = Activator.CreateInstance(tileType) as Tile; // «аменить на более производительный вариант
                //ToDo: Call base constructor and initialize
            }
        }
        return newTileArray;
...
/// <summary> Ѕазовый класс сетки.</summary>
public class Grid
{
    //Tiles collection
    private Tile[,] _tiles;
